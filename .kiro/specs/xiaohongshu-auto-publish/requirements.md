...
**用户故事**：作为内容运营人员，我想为每个浏览器窗口配置对应的飞书表格，以便每个账号自动发布自己表格中的笔记。

#### 接受标准

1. WHEN 用户配置窗口与表格映射时 THEN 系统 SHALL 保存浏览器窗口ID与飞书表格ID的一对一关系
2. WHEN 用户点击"加载笔记"时 THEN 系统 SHALL 根据映射关系从每个窗口对应的飞书表格读取待发布笔记
3. WHEN 系统显示笔记列表时 THEN 系统 SHALL 按窗口分组显示笔记，每个窗口显示其对应表格的笔记
4. WHEN 用户点击"开始发布"时 THEN 系统 SHALL 让每个浏览器窗口并行发布自己表格中的笔记
5. WHEN 某个窗口的表格没有待发布笔记时 THEN 系统 SHALL 显示该窗口无待发布内容
